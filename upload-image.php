    ...
    if ( $_FILES["file"]["size"] < ($max_size) )
    {
      ...
      else
      {
        ...
        else
        {
          ...
          $userquery=mysqli_query($conn,"INSERT INTO `Image` (`Imagelocation`,`UploaderID`) values ('$targetPath',$userid)");

          $imagequery=mysqli_query($conn,"Select * from `Image` where `Imagelocation`='$targetPath'");
          $imageresult=mysqli_fetch_array($imagequery);
          $imaged= array("imageid"=>$imageresult[0]);
          $imageid=json_encode($imaged);
          echo $imageid;
